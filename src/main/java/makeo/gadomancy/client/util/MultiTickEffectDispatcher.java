...
public class MultiTickEffectDispatcher {
    ...
    private static void handleBubbleSpawnings(World world) {
        ...
        while(it.hasNext()) {
            ...
            if(info.dimId != world.provider.getDimensionId()) {
                it.remove();
                continue;
            }
            info.renderTicks++;
    ...
    /*private static void handleWispyLineEffects(World currentRenderWorld, float partialTicks) {
        ...
        while(it.hasNext()) {
            ...
            if(info.dimId != currentRenderWorld.provider.getDimensionId()) {
                it.remove();
                continue;
            }
            info.renderTicks++;
    ...
    private static void handleVortexDiggingEffects(World currentRenderWorld) {
        ...
        while(it.hasNext()) {
            ...
            Thaumcraft.proxy.getFX().boreDigFx(info.tX, info.tY, info.tZ, info.oX, info.oY, info.oZ, info.blockInstance, info.meta);
        }
    }
