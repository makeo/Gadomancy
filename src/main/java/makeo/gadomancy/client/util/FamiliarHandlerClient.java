...
public class FamiliarHandlerClient {
    ...
    public static void processBoltPacket(PacketFamiliarBolt pkt) {
        ...
        ExFamiliarData data = clientFamiliars.get(p.getName());
        if(data == null) return;
        PartialEntityFamiliar fam = data.familiar;
        float y = (float) (fam.posY + 1.22F);
        if (!Minecraft.getMinecraft().thePlayer.getName().equalsIgnoreCase(pkt.owner)) {
            y += 0.2F;
        }
    ...
    public static void playerRenderEvent(EntityPlayer player, float partialTicks) {
        ...
        if(ENTITY_WISP == null) ENTITY_WISP = new EntityWisp(new FakeWorld(true));
        ...
        String currentPl = current.getName();
        String otherPl = player.getName();
        ...
        //ItemRenderFamiliar.renderEntityWispFor(fallbackRenderer, fam.owner.get(), ENTITY_WISP, diffX, diffY, diffZ, 0, partialTicks);
        GL11.glPopMatrix();
    }
    ...
    static {
        fallbackRenderer = new RenderWisp(Minecraft.getMinecraft().getRenderManager());
    ...
    public static class PartialEntityFamiliar {
        ...
        public void tick() {
            ...
            if(owner.get() == null) {
                ...
                if(owner.get().worldObj.provider.getDimensionId() != Minecraft.getMinecraft().getRenderViewEntity().worldObj.provider.getDimensionId()) {
