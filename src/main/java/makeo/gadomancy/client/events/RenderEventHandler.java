...
public class RenderEventHandler {
    ...
    private static final WandRenderingHandler WAND_RENDER_HANDLER = new WandRenderingHandler();
    ...
    public void on(DrawBlockHighlightEvent e) {
        ...
        if(e.currentItem.getItem() instanceof ItemWand) {
            ItemFocusBasic focus = ((ItemWand) e.currentItem.getItem()).getFocus(e.currentItem);
            if(focus == null || !(focus instanceof IArchitect)) {
                BlockPos pos = e.target.getBlockPos();
                IBlockState state = e.player.worldObj.getBlockState(pos);
                if(state.getBlock() != null && state.getBlock() == RegisteredBlocks.blockArcaneDropper) {
                    EnumFacing facing = EnumFacing.getFront(state.getBlock().getMetaFromState(state) & 7);
                        ...
                        for(int y = -1; y < 2; y++) {
                            for(int z = -1; z < 2; z++) {
                                coords.add(new BlockPos(
                                        pos.getX() + 2*facing.getFrontOffsetX() + x,
                                        pos.getY() + 2*facing.getFrontOffsetY() + y,
                                        pos.getZ() + 2*facing.getFrontOffsetZ() + z));
                            }
                        }
                    }
                    coords.add(new BlockPos(
                    ...
                    WAND_RENDER_HANDLER.handleArchitectOverlay(new ItemStack(ARCHITECT_ITEM), e.player, e.partialTicks, e.player.ticksExisted, e.target);
    ...
    public void worldRenderEvent(RenderWorldLastEvent event) {
        //ExtendedTypeDisplayManager.notifyRenderTick(); was for node stuff
