...
public class FXFlow {
    ...
    private void doParticles() {
        ...
        if(Minecraft.getMinecraft().theWorld.provider.getDimensionId() != origin.provider.getDimensionId()) {
            EffectHandler.getInstance().unregisterFlow(this);
            return;
        }
    ...
    public static class FXFlowBase extends EntityFX {
        ...
        public static void sheduleRender(Tessellator tessellator) {
            ...
            for(FXFlowBase fx : fxQueue) {
                ...
                fx.renderEssenceBase(tessellator.getWorldRenderer());
                tessellator.draw();
            }
        ...
        private void renderEssenceBase(WorldRenderer worldRenderer) {
            ...
            worldRenderer.pos(f11 - rendArg1 * f10 - rendArg4 * f10, f12 - rendArg2 * f10, f13 - rendArg3 * f10 - rendArg5 * f10).tex(0, 1);
            worldRenderer.pos(f11 - rendArg1 * f10 + rendArg4 * f10, f12 + rendArg2 * f10, f13 - rendArg3 * f10 + rendArg5 * f10).tex(1, 1);
            worldRenderer.pos(f11 + rendArg1 * f10 + rendArg4 * f10, f12 + rendArg2 * f10, f13 + rendArg3 * f10 + rendArg5 * f10).tex(1, 0);
            worldRenderer.pos(f11 + rendArg1 * f10 - rendArg4 * f10, f12 - rendArg2 * f10, f13 + rendArg3 * f10 - rendArg5 * f10).tex(0, 0);
        }
        ...
        public void renderParticle(WorldRenderer worldRenderer, Entity entityIn, float partialTicks, float par3, float par4, float par5, float par6, float par7) {
            if(MiscUtils.getPositionVector(Minecraft.getMinecraft().getRenderViewEntity()).distance(new Vector3(posX, posY, posZ)) > ModConfig.renderParticleDistance) return;
