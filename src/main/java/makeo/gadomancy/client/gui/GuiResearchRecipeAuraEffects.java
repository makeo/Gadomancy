...
public class GuiResearchRecipeAuraEffects extends GuiResearchRecipe {
    ...
    protected void genResearchBackground(int par1, int par2, float par3) {
        ...
        inj.setField("tooltip", null); //TODO ups
        ResearchPage[] pages = inj.getField("pages");
        int page = inj.getField("page");
                ...
                if ((mx >= tx) && (my >= ty) && (mx < tx + 40) && (my < ty + 40)) {
                    ...
                    if ((items != null) && (items.size() > 0)) {
                        ...
                        for (ItemStack item : items) {
                            ...
                            itemRender.renderItemAndEffectIntoGUI(/*this.mc.fontRendererObj, this.mc.renderEngine, InventoryUtils.cycleItemStack(item), mx + 8 + xcount * 17, 17 * ycount + (my - (4 + items.size() / 8 * 8)));
                            itemRender.renderItemOverlayIntoGUI(this.mc.fontRendererObj, /*this.mc.renderEngine, InventoryUtils.cycleItemStack(item), mx + 8 + xcount * 17, 17 * ycount + (my - (4 + items.size() / 8 * 8)), null);
