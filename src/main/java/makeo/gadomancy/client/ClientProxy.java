...
import makeo.gadomancy.client.render.entity.RenderEntityAuraCore;
...
public class ClientProxy extends CommonProxy {
    ...
    private void setupEntityRenderers() {
        RenderingRegistry.registerEntityRenderingHandler(EntityPermNoClipItem.class,
                new IRenderFactory<EntityPermNoClipItem>() {
            @Override
            public Render<? super EntityPermNoClipItem> createRenderFor(RenderManager manager) {
                return new RenderSpecialItem(manager, Minecraft.getMinecraft().getRenderItem());
            }
        });
        RenderingRegistry.registerEntityRenderingHandler(EntityAuraCore.class,
                new IRenderFactory<EntityAuraCore>() {
            @Override
            public Render<? super EntityAuraCore> createRenderFor(RenderManager manager) {
                return new RenderEntityAuraCore(manager, Minecraft.getMinecraft().getRenderItem());
            }
        });
    }
    ...
    public void postInitalize() {
        ...
        ResourceReloadListener.loadLanguageList();
    ...
    public Object getClientGuiElement(int ID, EntityPlayer player, World world, int x, int y, int z) {
        switch (ID) {
            ...
            case 1:
                return new InfusionClawGui(player.inventory, (IInventory) world.getTileEntity(new BlockPos(x, y, z)));
            case 2:
                return new ArcanePackagerGui(player.inventory, (IInventory) world.getTileEntity(new BlockPos(x, y, z)));
        }
        return null;
    }
