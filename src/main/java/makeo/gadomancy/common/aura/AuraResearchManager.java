...
public class AuraResearchManager {
    ...
    public static void tryUnlockAuraEffect(EntityPlayer player, Aspect aspect) {
        ...
        if(!ResearchHelper.isResearchComplete(player.getName(), Gadomancy.MODID.toUpperCase() + ".AURA_EFFECTS")) return;
        ...
        if(ResearchHelper.isResearchComplete(player.getName(), res)) return;
        //if(ResearchManager.isResearchComplete(player.getCommandSenderName(), res)) return;
        ResearchHelper.completeResearch(player, res);
    ...
    public static List<String> getKnowledge(EntityPlayer player) {
        ...
        for(Aspect a : AuraEffectHandler.registeredEffects.keySet()) {
            if(ResearchHelper.isResearchComplete(player.getName(), String.format(TC_AURA_RESEARCH_STR, a.getTag()))) {
