...
public final class MiscUtils {
    ...
    public static List<BlockPos> getPositionsAround(BlockPos blockPos) {
        List<BlockPos> coords = new ArrayList<BlockPos>();
        coords.add(new BlockPos(blockPos.getX(),     blockPos.getY(),     blockPos.getZ()));
        coords.add(new BlockPos(blockPos.getX() + 1, blockPos.getY(),     blockPos.getZ()));
        coords.add(new BlockPos(blockPos.getX(),     blockPos.getY(),     blockPos.getZ() + 1));
        coords.add(new BlockPos(blockPos.getX() - 1, blockPos.getY(),     blockPos.getZ()));
        coords.add(new BlockPos(blockPos.getX(),     blockPos.getY(),     blockPos.getZ() - 1));
        coords.add(new BlockPos(blockPos.getX(),     blockPos.getY() - 1, blockPos.getZ()));
        coords.add(new BlockPos(blockPos.getX(),     blockPos.getY() + 1, blockPos.getZ()));
        return coords;
    }
    ...
    public static NetworkRegistry.TargetPoint getTargetPoint(World world, TileEntity tileEntity, double range) {
        return getTargetPoint(world, range, new Vector3(tileEntity.getPos().getX(), tileEntity.getPos().getY(), tileEntity.getPos().getZ()));
    }

    public static NetworkRegistry.TargetPoint getTargetPoint(World world, double range, Vector3 target) {
        return new NetworkRegistry.TargetPoint(world.provider.getDimensionId(), target.getX(), target.getY(), target.getZ(), range);
    }
