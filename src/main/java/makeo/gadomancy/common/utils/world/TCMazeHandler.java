...
public class TCMazeHandler {
    ...
    public static void tick() {
        /*WorldServer w = DimensionManager.getWorld(ModConfig.);
        if (w != null) {
            if (!w.disableLevelSaving) w.disableLevelSaving = true;
            ...
            for (int i = 0; i < playerObjects.size(); i++) {
                ...
                if (!hasOpenSession(player)) {
                    ...
                    BlockPos pos = out.getSpawnCoordinate();
                    BlockPos topSolid = out.getTopSolidOrLiquidBlock(pos);
                    player.setPositionAndUpdate(topSolid.getX() + 0.5, topSolid.getY(), topSolid.getZ() + 0.5);
            ...
            while(it.hasNext()) {
                ...
                if (player.worldObj.provider.getDimensionId() != ModConfig.dimOuterId) { //If the player left our dim, but he should still be in the session, ...
    ...
    public static void handleBossFinish(TCMazeSession session) {
        ...
        world.setBlockState(new BlockPos(lX, 52, lZ), BlocksTC.eldritch.getStateFromMeta(3));
        //world.setBlock(lX, 52, lZ, BlocksTC.eldritch, 3, 3);
        //TODO setAdditionalEldrichPortal
        //world.setBlock(lX, 53, lZ, RegisteredBlocks.blockAdditionalEldrichPortal);
        GenCommon.genObelisk(world, new BlockPos(lX, 54, lZ));
    ...
    private static void setupSession(EntityPlayer player, WorldServer world) {
        ...
        int originDim = player.worldObj.provider.getDimensionId();
                    ...
                    for(int cX = 0; cX < 16; cX++) {
                        for(int cY = 51; cY < 61; cY++) {
                            for(int cZ = 0; cZ < 16; cZ++) {
                                TCMazeHandler.GEN.checkLightFor(EnumSkyBlock.BLOCK, new BlockPos((l.x << 4)+cX, cY, (l.z << 4)+cZ));
                            }
                        }
                    }
                }
