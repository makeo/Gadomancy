...
public class TCMazeSession {
    ...
    final void startSession() {
        ...
        for(CellLoc loc : chunksAffected.keySet()) {
            ...
            if(ws.theChunkProviderServer.id2ChunkMap.containsItem(k)) {
                Chunk c = ws.theChunkProviderServer.id2ChunkMap.getValueByKey(k);
                ws.theChunkProviderServer.loadedChunks.remove(c);
                ws.theChunkProviderServer.id2ChunkMap.remove(k);
            }
        }
        ...
        if(portalCell == null) {
            ...
            ResearchHelper.completeResearch(player, "ENTEROUTER"); //TODO confirm if still exists.
