...
public class ItemArcanePackage extends ItemFakeLootbag {
    ...
    public boolean setContents(ItemStack stack, List<ItemStack> items) {
        ...
        for (ItemStack item : items) {
            if (item != null) {
                InventoryUtils.insertStack(tempInv, item.copy(), EnumFacing.DOWN, true);
            }
        }
        ...
        ByteArrayOutputStream bout; //Does not need to be closed. has No-op method there.
        try {
            bout = new ByteArrayOutputStream();
            CompressedStreamTools.writeCompressed(compound, bout);
    ...
    public List<ItemStack> getContents(ItemStack stack) {
        ...
        if (stack.hasTagCompound()) {
            ...
            if(data.length > 0) {
                ...
                try {
                    ByteArrayInputStream bis = new ByteArrayInputStream(data); //Doesn't need to be closed.
                    NBTTagCompound compound = CompressedStreamTools.read(new DataInputStream(bis));
                    stackList = (NBTTagList) compound.getTag("data");
                } catch (Exception e) {
                    return contents;
                }
