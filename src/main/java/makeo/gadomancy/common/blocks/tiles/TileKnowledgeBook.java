 ...
 * TileKnowledgeBook
 */
public class TileKnowledgeBook extends SynchronizedTileEntity implements EntityPermNoClipItem.IItemMasterTile, IAspectContainer, ITickable {
    ...
    @Deprecated
    public static Map<IBlockState, Integer> knowledgeIncreaseMap = new HashMap<IBlockState, Integer>();
        ...
        for (int xx = -SURROUNDINGS_SEARCH_XZ; xx <= SURROUNDINGS_SEARCH_XZ; xx++) {
            for (int zz = -SURROUNDINGS_SEARCH_XZ;  zz <= SURROUNDINGS_SEARCH_XZ;  zz++) {
                ...
                for (int yy = -SURROUNDINGS_SEARCH_Y; yy <= SURROUNDINGS_SEARCH_Y; yy++) {
                    BlockPos absPos = getPos().add(xx, yy, zz);
                    IBlockState state = worldObj.getBlockState(absPos);
                    TileEntity te = worldObj.getTileEntity(absPos);
                    if(state.getBlock().equals(Blocks.bookshelf)) {
                        this.surroundingKnowledge += 1;
                    } else if(te != null && te instanceof IKnowledgeProvider) {
                        this.surroundingKnowledge += ((IKnowledgeProvider) te).getProvidedKnowledge(worldObj, absPos);
                    } else if(state.getBlock() instanceof IKnowledgeProvider) {
                        this.surroundingKnowledge += ((IKnowledgeProvider) state.getBlock()).getProvidedKnowledge(worldObj, absPos);
                    } else {
                        for (IBlockState sn : knowledgeIncreaseMap.keySet()) {
                            if(sn.equals(state)) {
    ...
    private void searchForCognitio() {
        if((ticksExisted & 31) == 0) {
            int drainRange = 4;
            EnumFacing[] toTry = EnumFacing.VALUES;
            for (EnumFacing dir : toTry) {
                if(dir == null) continue; //LUL should not happen...
                if(EssentiaHandler.drainEssentia(this, Aspect.MIND, dir, drainRange, 20)) {
    ...
    private void doResearchEffects() {
        if((ticksExisted & 15) == 0) {
            UtilsFX.doRuneEffects(Minecraft.getMinecraft().theWorld, getPos().add(0, -1, 0), 0);
        }
        if((ticksExisted & 31) == 0) {
            //TODO check sounds
    ...
    private void finishResearch() {
        ...
        PacketStartAnimation packet = new PacketStartAnimation(PacketStartAnimation.ID_SPARKLE_SPREAD, getPos());
    ...
    private NetworkRegistry.TargetPoint getTargetPoint(double radius) {
        ...
        double yCoord = getPos().getY();
        double zCoord = getPos().getZ();
        return new NetworkRegistry.TargetPoint(worldObj.provider.getDimensionId(), xCoord, yCoord, zCoord, radius);
    }
    ...
    private void tryVortexUnfinishedResearchNotes() {
        ...
        double yCoord = getPos().getY();
        double zCoord = getPos().getZ();
        float centerY = getPos().getY() + 0.4F;
        List entityItems = worldObj.getEntitiesWithinAABB(EntityItem.class,
                AxisAlignedBB.fromBounds(xCoord - 0.5, centerY - 0.5, zCoord - 0.5, xCoord + 0.5, centerY + 0.5, zCoord + 0.5).expand(8, 8, 8),
                new Predicate<EntityItem>() {
                    ...
                    public boolean apply(@Nullable EntityItem e) {
                        return !(e instanceof EntityPermanentItem) && !(e instanceof EntitySpecialItem) &&
                                e != null && e.getEntityItem() != null &&
                                e.getEntityItem().getItem() instanceof ItemResearchNotes &&
                                shouldVortexResearchNote(e.getEntityItem());
                    }
                });
        ...
        if(dummy.getDistanceToEntity(entity) < 1 && !worldObj.isRemote) {
            ...
            EntityPermNoClipItem item = new EntityPermNoClipItem(entity.worldObj, (float) (xCoord + 0.5F), centerY + 0.3F, (float) (zCoord + 0.5F),
                    storedResearchNote, (int) xCoord, (int) yCoord, (int) zCoord);
    ...
    private void applyMovementVectors(EntityItem entity) {
        ...
        double yCoord = getPos().getY();
        double zCoord = getPos().getZ();
        double var3 = (xCoord + 0.5D - entity.posX) / 15.0D;
        double var5 = (yCoord + 0.5D - entity.posY) / 15.0D;
        double var7 = (zCoord + 0.5D - entity.posZ) / 15.0D;
    ...
    public class FloatingBookAttributes {
        ...
        private void updateFloatingBook() {
            ...
            EntityPlayer entityplayer = TileKnowledgeBook.this.worldObj.getClosestPlayer((double)((float)TileKnowledgeBook.this.getPos().getX() + 0.5F), (double)((float)TileKnowledgeBook.this.getPos().getY() + 0.5F), (double)((float)TileKnowledgeBook.this.getPos().getZ() + 0.5F), 3.0D);

            if (entityplayer != null)
            {
                double d0 = entityplayer.posX - (double)((float)TileKnowledgeBook.this.getPos().getX() + 0.5F);
                double d1 = entityplayer.posZ - (double)((float)TileKnowledgeBook.this.getPos().getZ() + 0.5F);
