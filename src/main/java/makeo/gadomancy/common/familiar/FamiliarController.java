...
public class FamiliarController {
    ...
    private boolean doAttack(FamiliarAugment.FamiliarAugmentPair element, double damage, double range) {
        ...
        if (augment.equals(FamiliarAugment.SHOCK)) {
            ...
            for (EntityLivingBase entity : toAttack) {
                ...
                if (effectLevel == 3) {
                    entity.addPotionEffect(new PotionEffect(PotionSunScorned.instance.getId(), MiscUtils.ticksForMinutes(1), 2));
                }
            }
        } else if (augment.equals(FamiliarAugment.POISON)) {
            int potionLvl = 0;
    ...
    public static boolean consumeVisFromInventory(EntityPlayer player, AspectList cost, boolean doit) {
        ...
        for (int a = player.inventory.mainInventory.length - 1; a >= 0; a--) {
            ...
            if ((item != null) && ((item.getItem() instanceof ItemWand))) {
                boolean done = ((ItemWand) item.getItem()).consumeAllVis(item, player, cost, doit, true);
                if (done) {
                    return true;
                }
            }
    ...
    private void attack(EntityLivingBase toAttack, double damage, int boltType) {
        ...
        PacketFamiliarBolt bolt = new PacketFamiliarBolt(owningPlayer.getCommandSenderEntity().getName(), (float) toAttack.posX, (float) toAttack.posY, (float) toAttack.posZ, boltType, true);
        PacketHandler.INSTANCE.sendToAllAround(bolt, new NetworkRegistry.TargetPoint(toAttack.worldObj.provider.getDimensionId(), owningPlayer.posX,
    ...
    private List<EntityLivingBase> getAttackableEntities(double rad) {
        ...
        List<EntityLivingBase> entities = owningPlayer.worldObj.getEntitiesWithinAABB(EntityLivingBase.class,
                AxisAlignedBB.fromBounds(x - 0.5, y - 0.5, z - 0.5, x + 0.5, y + 0.5, z + 0.5).expand(rad, rad, rad));
