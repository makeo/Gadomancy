...
public class PseudoResearchItem extends SimpleResearchItem {
    ...
    private PseudoResearchItem(ResearchItem original, String key, int col, int row, ResourceLocation icon) {
        super(PSEUDO_PREFIX + key, col, row, original.getComplexity(), icon, new AspectList());
    ...
    public static ResearchItem create(String original, int col, int row) {
        ...
        if(item != null) {
            ...
            if(item.icon_resource == null) {
                pseudo = new PseudoResearchItem(item, item.key, col, row, item.icon_item);
            } else {
                pseudo = new PseudoResearchItem(item, item.key, col, row, item.icon_resource);
            }
