...
public class PacketSyncData implements IMessage, IMessageHandler<PacketSyncData, IMessage> {
    ...
    public void fromBytes(ByteBuf buf) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            buf.readBytes(abyte);
            try {
                ByteArrayInputStream bis = new ByteArrayInputStream(abyte); //Doesn't need to be closed.
                cmp = CompressedStreamTools.read(new DataInputStream(bis));
            } catch (IOException e) {
    ...
    public void toBytes(ByteBuf buf) {
        ...
        for(String key : data.keySet()) {
            ...
            try {
                ...
                CompressedStreamTools.writeCompressed(cmp, bout);
                abyte = bout.toByteArray();
            } catch (IOException e) {
